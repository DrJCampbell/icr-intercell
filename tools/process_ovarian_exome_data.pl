...
my ($help, $ovarian_mut_data, $mut_freqs, $genes, $cell_lines, $output_genes_list, $mut_cons_classes, $output);

GetOptions (
  "ovarian_muts=s" => \$ovarian_mut_data,
...
$output = "cosmic_mutation_data_table" unless defined $output;
...
# ====================================================== #
# parse the ovarian exome file and extract mutation info
# ====================================================== #
...
my %ovca_truncs;		# store truncating counts for cell*gene
my %ovca_rec_mis;	# store recurrent missense counts for cell*gene
my %ovca_other;		# store other counts for cell*gene

if(defined $ovarian_mut_data){
	# Open and read mutations file
	open OVCA, "< $ovarian_mut_data" or die "Can't read mutations file $ovarian_mut_data: $!\n";
	...
	while(<OVCA>){
	  ...
	  my $ovca_key = "$standard_cell_line\t$standard_gene";
	  ...
	  if($mutation_consequences{$var_class} eq "trunc"){	# if mutation is truncating
		$ovca_truncs{$ovca_key} ++;							# increment cellline+gene truncating count
	  }
	  ...
	  if(exists $details_table{$ovca_key}{"ovca_mut"}){
		$details_table{$ovca_key}{"ovca_mut"} .=  "; $prot_change";
		$details_table{$ovca_key}{"ovca_type"} .=  "; $mutation_consequences{$var_class}";
		if(exists($mut_freqs{$davoli_key})){
		  $details_table{$ovca_key}{"ovca_davoli_freq"} .= "; $mut_freqs{$davoli_key}";
		}
		else{
		  $details_table{$ovca_key}{"ovca_davoli_freq"} .= "; NA";
		}
	  }
	  else{
		$details_table{$ovca_key}{"ovca_mut"} =  $prot_change;
...
while(my ($seen_cell_line, $cell_line_seen_in_dataset) = each  %master_cell_lines_seen){
  ...
  foreach my $seen_gene (@genes_seen){
    ...
    if(exists($ovca_truncs{$hash_key})){
      $trunc = 1;
    }
    
    if(exists($ovca_rec_mis{$hash_key})){
      $rec_mis = 1;
    }
    
    if(exists($ovca_other{$hash_key})){
      $other = 1;
    }
...
foreach my $details_key (@details_keys){
  ...
  print DETAILS "\tOVCA (exome)";

# ovca_type
  if(exists $details_table{$details_key}{"ovca_type"}){
    chomp($details_table{$details_key}{"ovca_type"});
    print DETAILS "\t$details_table{$details_key}{'ovca_type'}";
  }
  else{
    print DETAILS "\tNA";
  }
...
# ovca_mut
  if(exists $details_table{$details_key}{"ovca_mut"}){
    chomp($details_table{$details_key}{'ovca_mut'});
    print DETAILS "\t$details_table{$details_key}{'ovca_mut'}";
  }
  else{
    print DETAILS "\tNA";
  }
...
# ovca_davoli_freq
  if(exists $details_table{$details_key}{"ovca_davoli_freq"}){
    chomp($details_table{$details_key}{"ovca_davoli_freq"});
    print DETAILS "\t$details_table{$details_key}{'ovca_davoli_freq'}";
  }
  else{
    print DETAILS "\tNA";
  }
...
# ---------------------------------- #
#  process_ovarian_data.pl
#  James Campbell (jamesc\@icr.ac.uk)
# ---------------------------------- #

Usage:
perl process_ovarian_data.pl [options]

Options:
  --help                    Display this message and quit
  --ovarian_muts            Path to the ovarian mutations file [required]
